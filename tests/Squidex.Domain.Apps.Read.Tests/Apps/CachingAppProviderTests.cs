...
namespace Squidex.Domain.Apps.Read.Apps
{
    public class CachingAppProviderTests
    {
        ...
        [Fact]
        public async Task Should_clear_cache_for_name_after_update_pattern_event()
        {
            A.CallTo(() => repository.FindAppAsync(appId.Name))
                .Returns(appV2);
            A.CallTo(() => repository.FindAppAsync(appId.Name))
            ...
            sut.On(Envelope.Create(new AppPatternUpdated { AppId = appId, Name = "Pattern1", Pattern = "Pattern1", OriginalName = "Pattern" }).To<IEvent>()).Wait();
