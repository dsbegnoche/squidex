...
namespace Squidex.Domain.Apps.Write.Contents
{
    public class ContentCommandMiddleware : ICommandMiddleware
    {
        ...
        protected Task On(DeclineContent command, CommandContext context)
        {
        ...
        private async Task ValidateAsync(ContentDataCommand command, Func<string> message, bool enrich = false)
        {
            ...
            Guard.NotNull<NullReferenceException>(taskForSchema.Result.Schema, $"'{nameof(taskForSchema.Result.Schema)}' cannot be null. A schema could not be found by '{command.SchemaId.Id}'");
